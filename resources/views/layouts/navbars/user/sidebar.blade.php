...
<nav x-data="{ sidebarOpen: window.innerWidth >= 1024 }" x-init="window.addEventListener('resize', () => sidebarOpen = window.innerWidth >= 1024)" class="fixed top-0 left-0 z-50 my-5 ms-4">
    ...
    <div class="p-2 h-10 bg-purple-200 shadow rounded-2xl block lg:hidden">
        <button @click="sidebarOpen = !sidebarOpen" class="text-gray-600 hover:text-purple-100 focus:outline-none">
    ...
    <div x-show="sidebarOpen" x-cloak @click.away="if (window.innerWidth < 1024) sidebarOpen = false"
        ...
        <div class="flex justify-between items-center mb-4">
            <a href="{{ route('admin.dashboard') }}" class="text-2xl font-bold tracking-wide">
                SAFE<span class="text-purple-400">SHIELD</span>
            </a>
            <button @click="sidebarOpen = false" class="text-gray-500 hover:text-purple-300 focus:outline-none lg:hidden">
                ...
                @if (isset($menu['submenu']))
                    <li x-data="{ open: false }">
                        <a href="#" @click.prevent="open = !open"
                            class="flex justify-between items-center p-2 rounded hover:bg-purple-100">
                            ...
                            @foreach ($menu['submenu'] as $submenu)
                                <li>
                                    <a href="{{ route($submenu['route']) }}"
                                        class="block p-2 rounded hover:bg-purple-100 {{ request()->routeIs($submenu['route']) ? 'bg-purple-200' : '' }}">
                ...
                @else
                    <li>
                        <a href="{{ route($menu['route']) }}"
                            class="block p-2 rounded hover:bg-purple-100 {{ request()->routeIs($menu['route']) ? 'bg-purple-200' : '' }}">
