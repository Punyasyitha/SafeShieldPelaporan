<x-app-layout>
    <div class="w-full bg-white shadow-lg rounded-lg p-6">
        <div class="overflow-x-auto">
            ...
            <div class="overflow-x-auto">
                <table id="kategoriTable" class="min-w-full border-gray-300 text-sm w-full">
                    <thead>
                        <tr class="bg-gray-100 text-sm leading-normal">
                            <th class="py-3 px-6 text-left w-10">No</th>
                            <th class="py-3 px-6 text-left w-40">ID Kategori</th>
                            <th class="py-3 px-6 text-left w-64">Nama Kategori</th>
                    ...
                    <tbody>
                        @foreach ($list as $ktg)
                            <tr class="border-b border-gray-300">
                                ...
                                <td class="py-3 px-6">{{ $ktg->idkategori }}</td>
                                <td class="py-3 px-6">{{ $ktg->nama_kategori }}</td>
                                <td class="py-3 px-6 flex flex-wrap gap-2">
                                    ...
                                    <form action="{{ route('master.kategori.delete', encrypt($ktg->idkategori)) }}"
                                        method="POST" class="delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="bg-red-500 hover:bg-red-600 text-white py-1 px-3 rounded delete-btn"
                                            data-id="{{ encrypt($ktg->idkategori) }}">
